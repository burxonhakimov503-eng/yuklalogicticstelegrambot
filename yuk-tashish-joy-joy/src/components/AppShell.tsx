...
export function AppShell() {
  ...
  useEffect(() => {
    if (!isPublic && !isLoggedIn) nav({ to: "/onboarding" });
    else if (loc.pathname === "/" && isLoggedIn) nav({ to: "/home" });
    else if (loc.pathname === "/" && !onboarded) nav({ to: "/onboarding" });
    else if (loc.pathname === "/" && onboarded && !isLoggedIn) nav({ to: "/login" });
  }, [loc.pathname, isLoggedIn, onboarded, isPublic, nav]);

  const showNav = isLoggedIn && !isPublic;
      ...
      <div className="max-w-md mx-auto bg-surface min-h-screen relative">
        ...
        {showAI && (
          ...
          >
            ...
            <span className="text-sm font-semibold leading-tight">Sun'iy<br />intellekt</span>
          </Link>
        )}
        {showNav && <BottomNav />}
      </div>
